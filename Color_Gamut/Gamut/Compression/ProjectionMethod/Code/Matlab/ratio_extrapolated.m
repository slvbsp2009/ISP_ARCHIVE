...
plot(x(1),y(1),'Marker','x', 'Color', 'black', 'LineWidth', 2)  


% acquiring extrapolated point's coordinates 

[flag, xr, yr] = ratio_longline(x(1), y(1)) 

plot(xr, yr, 'Marker', 's', 'Color', 'yellow', 'LineWidth', 2) 
...
D = calc_distance(xD, yD, TWPx, TWPy) 
d = calc_distance(xd, yd, TWPx, TWPy) 
    
xdist = calc_distance(x(1), y(1), TWPx, TWPy)
...
xc = ((1 - cr) * x(2)) + (cr * x(1)) 
yc = ((1 - cr) * y(2)) + (cr * y(1)) 
plot(xc, yc, 'Marker','+', 'Color', 'blue', 'LineWidth', 2) 
% line( [x(1), xc], [y(1), yc], 'LineStyle', '--', 'Color', 'blue', 'LineWidth', 2)