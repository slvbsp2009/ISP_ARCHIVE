...
[xq, yq] = readRGB(image_name); 

length(xq) 
length(yq) 

% Acquiring points that are within the HD Gamut (and on it's border)  
...
% % Acquiring points that are within the alpha HD Gamut (and on it's border) 
[in,on] = inpolygon(xcoord,ycoord, xahd,yahd); 

inon = in;                                     % Combine ‘in’ And ‘on’ 
idx = find(inon(:));                           % Linear Indices Of ‘inon’ Points 
...
newRGB = createRGB(xq, yq, image_name); 
savename = ['C:/Users/HP-PC/Documents/MATLAB/GamutMapping/res/res',image_name,'alpha',num2str(alpha),'.png']; 
imwrite(newRGB, savename, 'png'); 

savenameC = ['C:/Users/HP-PC/Documents/MATLAB/GamutMapping/res/res',image_name,'alpha',num2str(alpha),'C.png'];
saveas(figure1, savenameC, 'png'); 
